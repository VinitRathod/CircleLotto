    ...
    public function toArray(Request $request)
    {
        ...
        foreach ($this['circles'] as $key => $value) {
            if ($value['circle'] != null) {
                ...
                $resource['circles'][$key]['user_draw_numbers_count'] = DrawNumbers::where('circle_id', $value['circle_id'])->where('user_id', Auth::id())->count();
                $resource['circles'][$key]['total_circle_draw_numbers'] = DrawNumbers::where('circle_id', $value['circle_id'])->count();
